...
class clientAA(Client):
    def __init__(self, args, id, train_samples, test_samples, **kwargs):
        ...
        print(f'self.aa_params: {self.aa_params}')
        
        self.clip_model_object = CLIPModelWithAttentionAdapter(model_id=args.model_id, home_dir=args.home_dir, aa_params=self.aa_params).to(args.device)
        ...
        self.optimizer = torch.optim.Adam([p for name, p in self.aa.named_parameters() if p.requires_grad], lr=self.learning_rate,betas=(0.9,0.98),eps=1e-6,weight_decay=0.2)
...
#         auc = metrics.roc_auc_score(y_true, y_prob, average='micro')
        ...
        with torch.no_grad():
            ...
            for i, (images, target, texts) in enumerate(testloaderfull):
                ...
                # added adapter------------------------------------
                image_features_attn = self.aa(image_features)
                image_features = torch.mul(
                    image_features_attn, image_features)
                #--------------------------------------------------
