...
import copy
import random
from flcore.clients.clientvmlc import clientVMLC
...
#         print(f'args.model: {args.model}')
    ...
    def train(self):
        for i in range(self.global_rounds + 1):
            ...
            self.aggregate_parameters_vm()
            self.aggregate_parameters_lc()
        ...
        self.save_results()
        self.save_global_model_vm()
        self.save_global_model_lc()
    ...
    def send_models(self):
        # sending the IE and LC model
        assert (len(self.clients) > 0)

        for client in self.clients:
            start_time = time.time()
            
            global_vm_params = self.global_model_vm
    ...
    def receive_models(self):
        # Receive VM and LC model from each client
        ...
        for client in active_clients:
            ...
            if client_time_cost <= self.time_threthold:
                ...
                self.uploaded_models_vm.append(client.vm)
                self.uploaded_models_lc.append(client.lc)
...
#             print(f"Shape of '{key}': {tensor.shape}")
    ...
    def save_global_model_vm(self):
        model_path = os.path.join("model_vm", self.dataset)
    ...
    def aggregate_parameters_vm(self):
        ...
        self.global_model_vm = copy.deepcopy(self.uploaded_models_vm[0])
        for param in self.global_model_vm.parameters():
            param.data.zero_()
    ...
    def aggregate_parameters_lc(self):
        assert (len(self.uploaded_models_lc) > 0)

        self.global_model_lc = copy.deepcopy(self.uploaded_models_lc[0])
        for param in self.global_model_lc.parameters():
            param.data.zero_()
    ...
    def add_parameters_vm(self, w, client_model):
        for server_param, client_param in zip(self.global_model_vm.parameters(), client_model.parameters()):
            server_param.data += client_param.data.clone() * w
            
    def add_parameters_lc(self, w, client_model):
        for server_param, client_param in zip(self.global_model_lc.parameters(), client_model.parameters()):
            server_param.data += client_param.data.clone() * w
